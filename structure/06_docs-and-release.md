...
| npm registry | `@tungninh/aura-ai@<version>` does not exist before publish, then exists after publish with the requested dist-tag. |
...
```bash
npm view @tungninh/aura-ai@<version> version
...
npm install -g ./tungninh-aura-ai-*.tgz
ocx help
```
